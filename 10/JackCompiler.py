...
def create_vm_file(files_lst):
    """
    This function goes through each file in an array and opens a vm file.
    ...
    for file in files_lst:
        vm_file = os.path.basename(file).replace("jack", "vm")
        # replace the older vm file with the same name if exists.
        if os.path.isfile(vm_file):
            os.remove(vm_file)
        # open the vm file and write the compiled command to it.
        output_file = open(vm_file, "a")
